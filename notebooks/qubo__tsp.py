...
def _(
	...
	if route_labels is None:
		...
		_report.append(
			mo.md(rf"""Tour: ${_sequence}$ with total cost ${int(_total_cost)}$.""")
		)
	mo.vstack(_report)
	return
...
@app.cell(hide_code=True)
def _():
	from foundation import Graph, optimise
...
@app.cell(hide_code=True)
def _(np):
	def build_tsp_qubo(costs: np.ndarray, penalty: float):
	...
	return assignment_as_markdown, build_tsp_qubo, decode_tour, tour_cost
